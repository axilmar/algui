...
namespace algui {
    ...
    public:
        ...
        /** Node enabled event; fired by the node that became enabled; does not bubble. */
        static const std::string enabled;

        /** Node disabled event; fired by the node that became disabled; does not bubble. */
        ...
        /** mouse entered; fired when the mouse enters a node's area, including any children. */
        static const std::string mouseEnter;

        /** mouse moved; fired when the mouse moves over a node's area, including any children. */
        static const std::string mouseMove;

        /** mouse left; fired when the mouse leaves node's area. */
        static const std::string mouseLeave;

        /** mouse button pressed; fired when the mouse is pressed over a node. */
        static const std::string mouseButtonDown;

        /** mouse button held pressed; fired when the mouse is pressed continuously over a node. */
        static const std::string mouseButtonHeldDown;

        /** mouse button released; fired when the mouse is released over a node. */
        static const std::string mouseButtonUp;

        /** mouse wheel; fired when the mouse wheel is rolled over a node. */
        ...
        /** mouse entered while in drag and drop. */
        static const std::string dragEnter;

        /** mouse moved while in drag and drop. */
        static const std::string drag;

        /** mouse left while in drag and drop. */
        static const std::string dragLeave;

        /** mouse released while in drag and drop. */
        static const std::string drop;

        /** a key was pressed when the node has the input focus. */
        static const std::string keyDown;

        /** a key was released when the node has the input focus. */
        static const std::string keyUp;

        /** a character was created from a key press, while the node has the input focus. */
        static const std::string keyChar;

        /** a key was pressed but the node with the input focus ignored it.
            Fired only in the bubble phase. */
        ...
        /** a timer event; does not have a phase (so it uses phase 0). */
